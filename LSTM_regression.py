...
if frame == "pytorch":
    from src.LSTM_Model import train, predict


class Config:
    # 数据参数
    feature_columns = [1, 2]  # 要作为feature的列，按原数据从0开始计算，也可以用list 如 [2,4,6,8] 设置
    ...
    # 路径参数
    train_data_path = "../dataset/sentimentDaily-HSIexpert.csv"
...
class Data:
    ...
    def get_test_data(self, return_label_data=False):
        feature_data = self.norm_data[int(self.data_num * 0.15):]
        print('test data length :' + str(feature_data.shape[0]))
        ...
        if return_label_data:  # 实际应用中的测试集是没有label数据的
            label_data = self.norm_data[int(self.data_num * 0.15) + self.start_num_in_test:, self.config.label_in_feature_index]
...
def draw(config: Config, origin_data: Data, logger, predict_norm_data: np.ndarray):
    label_data = origin_data.data[int(origin_data.data_num * 0.15) + origin_data.start_num_in_test:,
    ...
    logger.info("The mean squared error of stock {} is ".format(label_name) + str(loss_norm))
    # label_X = range(origin_data.data_num - int(origin_data.data_num * 0.15) - origin_data.start_num_in_test)
    label_X = range(origin_data.data_num - int(origin_data.data_num * 0.15) - origin_data.start_num_in_test)
    ...
    for i in range(label_column_num):
        ...
        origin_data = label_data[:, i]
        prediction_data = predict_data[:, i]
    for i in range(len(origin_data) - 1):
        if origin_data[i + 1] >= origin_data[i] and prediction_data[i + 1] >= prediction_data[i]:
            TP = TP + 1
        if origin_data[i + 1] < origin_data[i] and prediction_data[i + 1] < prediction_data[i]:
            TN = TN + 1
        if origin_data[i + 1] >= origin_data[i] and prediction_data[i + 1] < prediction_data[i]:
    ...
    print("real negative    " + str(FP) + "     |    " + str(TN))
    Pre = TP / (TP + FP)
    Re = TP / (TP + FN)
    f1 = Pre * Re / (2 * (Pre + Re))
...
'''
    if not sys.platform.startswith('linux'):  # 无桌面的Linux下无法输出，如果是有桌面的Linux，如Ubuntu，可去掉这一行
        for i in range(label_column_num):
            ...
            plt.plot(label_X, label_data[:, i],'bo-', label=' HSI data',color='r')
            plt.plot(predict_X, predict_data[:, i], 'bo-',label='Prediction')
            plt.title("Predict stock {} price with {} - normal group".format(label_name[i], config.used_frame))

            plt.legend()
            plt.xlabel('Prediction to next day', size=10)
            plt.ylabel('Prediction and HSI data', size=10)
            if config.do_figure_save:
                plt.savefig(
                    config.figure_save_path + "{}predict_{}_with_normal_group.png".format(config.continue_flag, label_name[i],
                                                                                config.used_frame))
...
if __name__ == "__main__":
    ...
    parser.add_argument("-e", "--epoch", default=30, type=int, help="epochs num")
